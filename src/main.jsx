...
function App(){
  ...
  return <HomePage state={state} go={go} lessons={lessons} selectedLesson={selectedLesson} setSelectedLesson={setSelectedLesson} showAllWords={showAllWords} setShowAllWords={setShowAllWords} />;
}
...
function HomePage({state,go,lessons,selectedLesson,setSelectedLesson}){
 ...
 return <Shell state={state} go={go}>
  <section className="container">
   ...
   <div className="continue-card" onClick={()=>{setSelectedLesson(next.id);go("lesson")}}><div className="lesson-icon">{(() => { const Icon = next.icon; return <Icon size={25}/>; })()}</div><div className="grow"><small>Lesson {next.id}</small><b>{next.title}</b><span>{next.hi} • {next.words.length} words</span></div><ChevronRight/></div>
   <div className="section-head"><h3>Your Course</h3><span className="muted">{completed}/{lessons.length}</span></div>
   <div className="course-list">{lessons.map(l=>{const done=state.completed.includes(l.id); return <button key={l.id} className={"lesson-row "+(done?"done":"")} onClick={()=>{setSelectedLesson(l.id);go("lesson")}}><span className="mini-icon">{(() => { const Icon = l.icon; return <Icon size={18}/>; })()}</span><div className="grow"><b>{l.id}. {l.title}</b><small>{l.hi}</small></div>{done?<span className="check"><Check size={15}/></span>:<ChevronRight size={18}/>}</button>})}</div>
...
function Quiz({state,update,go}){ const [q,setQ]=useState(0),[score,setScore]=useState(0),[selected,setSelected]=useState(null),[finished,setFinished]=useState(false);
 ...
 const next=()=>{if(q===quizBank.length-1){const final=score+(selected===item[1]?1:0);const pct=Math.round((final/quizBank.length)*100);update({quizBest:Math.max(state.quizBest,pct),xp:state.xp+Math.round(pct/5)});setFinished(true)}else{setQ(q+1);setSelected(null)}};
